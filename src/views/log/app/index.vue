<template>
  <div class="container">
    <Breadcrumb :items="['menu.log', 'menu.log.app']" />
    <a-card class="general-card" :title="t('log.app.title')">
      <a-row>
        <a-col :flex="1">
          <a-form
            :model="searchForm"
      ...
      >
        <template #columns>
          ...
          <a-table-column
            :title="t('log.columns.location')"
            data-index="location"
          />
          <a-table-column
            :title="t('log.columns.device')"
            data-index="device"
          />
...
<script lang="ts" setup>
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const { list, total } = await logApi.getAppLogList(searchForm);
