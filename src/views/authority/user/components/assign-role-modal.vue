...
<script lang="ts" setup>
  ...
  const loadData = async () => {
    try {
      ...
      currentRoles.value = allEnabledRoles.filter((role) =>
        currentRoleIds.has(role.id)
      );

      // 构建可用角色列表
      availableRoles.value = allEnabledRoles.filter(
  ...
  const handleRemove = () => {
    ...
    const selectedRoles = currentRoles.value.filter((role) =>
      checkedKeys.value.includes(role.id)
    );

    availableRoles.value = [...availableRoles.value, ...selectedRoles];
    currentRoles.value = currentRoles.value.filter(
      (role) => !checkedKeys.value.includes(role.id)
    );
